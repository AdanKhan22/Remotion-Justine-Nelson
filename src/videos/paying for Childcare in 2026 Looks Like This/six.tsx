...
/**
 * SCENE 6: CENTRAL VALLEY, CALIFORNIA (180 frames @ 30fps = 6.0s)
 * - Dedicated Standalone Feature (Host deep dive / TikTok clip intro)
 * - Starts at Full US Map (zoom: 1.0)
 * - Highlights California in Ice Blue (#C1E1E6)
 * - Camera eases in with West Coast framing & PiP headroom (zoom: 1.75, txFactor: 0.22)
 * - Pin drops, DM Card opens, price rolls live from $2,800 -> $2,700
 */
export const Scene6_CaliforniaNurse: React.FC = () => {
  ...
  // Source of Truth Data: Entry #6
  const location = LOCATIONS.california_nurse;
  ...
  // Moderated zoom (1.75x) with West Coast shift to leave ample PiP space on the right
  ...
  const currentTy = interpolate(
    zoomProgress,
    [0, 1],
    [0, CAMERA_PRESETS.california.tyFactor * height]
  );
  ...
  // 6. LIVE PRICE COUNT-DOWN/ROLL ($2,800 from Michigan -> $2,700 for CA Nurse)
  const animatedPrice = Math.round(
    interpolate(frame, [92, 116], [2800, 2700], {
      extrapolateLeft: 'clamp',
      extrapolateRight: 'clamp',
  ...
  return (
    <AbsoluteFill style={{backgroundColor: BRAND_COLORS.canvasBg, overflow: 'hidden'}}>
      {/* SVG OVERFLOW SAFETY: Prevents map viewBox edge slicing */}
      <style>{`
        svg, .rsm-svg {
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      />
                      {/* High-visibility demographic badge */}
                      <span
                        style={{
...
export default Scene6_CaliforniaNurse;