...
/**
 * SCENE 12: CHARLOTTESVILLE, VIRGINIA (HANNAH) (150 frames @ 30fps = 5.0s)
 * - Final Standalone Feature: Daycare wiping out take-home income ($3,000/mo)
 * - Starts at Full US Map (zoom: 1.0)
 * - Highlights Virginia in Ice Blue (#C1E1E6)
 * - Camera eases in with PiP headroom (zoom: 1.65, txFactor: -0.28)
 * - Pin drops, DM Card opens, price rolls up from $2,600 -> $3,000
 */
export const Scene12_CharlottesvilleHannah: React.FC = () => {
  ...
  // Source of Truth Data: Entry #12
  const location = LOCATIONS.charlottesville;
  ...
  // Balanced East Coast framing to leave negative space on the right for PiP
  const targetZoom = 1.65;
  const targetTxFactor = -0.28;
  ...
  // 6. LIVE PRICE COUNT-UP ($2,600 from Bay Area -> $3,000 for Charlottesville)
  const animatedPrice = Math.round(
    interpolate(frame, [92, 116], [2600, 3000], {
      extrapolateLeft: 'clamp',
      extrapolateRight: 'clamp',
...
export default Scene12_CharlottesvilleHannah;