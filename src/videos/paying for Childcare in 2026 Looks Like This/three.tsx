...
/**
 * SCENE 3: ROCKVILLE, MARYLAND (150 frames @ 30fps = 5.0s)
 * - Starts at Full US Map (zoom: 1.0)
 * - Highlights Maryland in Ice Blue (#C1E1E6)
 * - Camera eases in with PiP headroom (zoom: 1.75, txFactor: -0.28)
 * - Pin drops, DM Card opens, price rolls up from $1,460 -> $1,720
 */
export const Scene3_RockvilleMaryland: React.FC = () => {
  ...
  // Source of Truth Data: Entry #3
  const location = LOCATIONS.rockville;
  ...
  // Moderated zoom (1.75x) with East Coast pan (-0.28) to prevent clipping and keep PiP room
  ...
  const currentTy = interpolate(
    zoomProgress,
    [0, 1],
    [0, CAMERA_PRESETS.maryland.tyFactor * height]
  );
  ...
  // 6. LIVE PRICE COUNT-UP ($1,460 from Littleton -> $1,720 for Rockville)
  const animatedPrice = Math.round(
    interpolate(frame, [92, 116], [1460, 1720], {
      extrapolateLeft: 'clamp',
      extrapolateRight: 'clamp',
  ...
  return (
    <AbsoluteFill style={{backgroundColor: BRAND_COLORS.canvasBg, overflow: 'hidden'}}>
      ...
      <USMapRig
        ...
        currentTx={currentTx}
        currentTy={currentTy}
        transformOrigin={CAMERA_PRESETS.maryland.origin}
...
export default Scene3_RockvilleMaryland;