...
/**
 * SCENE 5: GRAND RAPIDS, MICHIGAN (150 frames @ 30fps = 5.0s)
 * - Starts at Full US Map (zoom: 1.0)
 * - Highlights Michigan in Ice Blue (#C1E1E6)
 * - Camera pushes in with PiP headroom (zoom: 1.75)
 * - Demographic tag enlarged to 17.5*scale for high mobile visibility
 * - Pin drops, DM Card opens, price rolls up from $2,600 -> $2,800
 */
export const Scene5_GrandRapidsMichigan: React.FC = () => {
  ...
  // Source of Truth Data: Entry #5
  const location = LOCATIONS.grandrapids;
  ...
  // Moderated zoom (1.75x) to maintain room for Premiere Pro PiP
  ...
  const currentTx = interpolate(
    zoomProgress,
    [0, 1],
    [0, CAMERA_PRESETS.michigan.txFactor * width]
  );
  const currentTy = interpolate(
    zoomProgress,
    [0, 1],
    [0, CAMERA_PRESETS.michigan.tyFactor * height]
  );
  ...
  // 6. LIVE PRICE COUNT-UP ($2,600 from Iowa -> $2,800 for Grand Rapids)
  const animatedPrice = Math.round(
    interpolate(frame, [92, 116], [2600, 2800], {
      extrapolateLeft: 'clamp',
      extrapolateRight: 'clamp',
  ...
  return (
    <AbsoluteFill style={{backgroundColor: BRAND_COLORS.canvasBg, overflow: 'hidden'}}>
      ...
      <USMapRig
        ...
        currentTx={currentTx}
        currentTy={currentTy}
        transformOrigin={CAMERA_PRESETS.michigan.origin}
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      />
                      {/* Bold, Enlarged Demographic Badge */}
                      <span
                        style={{
                          ...
                          fontSize: 17.5 * scale, // Enlarged mobile-first scale
                          fontWeight: 800,
                          letterSpacing: '0.01em',
                        }}
                      >
...
export default Scene5_GrandRapidsMichigan;